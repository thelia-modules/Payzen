...
namespace Payzen\Model\Map;

use Payzen\Model\PayzenConfig;
 ...
 *
 */
class PayzenConfigTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Payzen.Model.Map.PayzenConfigTableMap';
     ...
     */
    const TABLE_NAME = 'payzen_config';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\Payzen\\Model\\PayzenConfig';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Payzen.Model.PayzenConfig';
     ...
     */
    const NAME = 'payzen_config.NAME';

    /**
     * the column name for the VALUE field
     */
    const VALUE = 'payzen_config.VALUE';
    ...
    {
        // attributes
        $this->setName('payzen_config');
        $this->setPhpName('PayzenConfig');
        $this->setClassName('\\Payzen\\Model\\PayzenConfig');
        $this->setPackage('Payzen.Model');
    ...
    {
        return $withPrefix ? PayzenConfigTableMap::CLASS_DEFAULT : PayzenConfigTableMap::OM_CLASS;
    }
     ...
     * @return array (PayzenConfig object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = PayzenConfigTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = PayzenConfigTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + PayzenConfigTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = PayzenConfigTableMap::OM_CLASS;
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            PayzenConfigTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = PayzenConfigTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = PayzenConfigTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(PayzenConfigTableMap::NAME);
            $criteria->addSelectColumn(PayzenConfigTableMap::VALUE);
        } else {
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(PayzenConfigTableMap::DATABASE_NAME)->getTable(PayzenConfigTableMap::TABLE_NAME);
    }
    ...
    /**
     * Performs a DELETE on the database, given a PayzenConfig or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or PayzenConfig object or primary key or array of primary keys
    ...
    {
        return PayzenConfigQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a PayzenConfig or Criteria object.
     *
     * @param mixed               $criteria Criteria or PayzenConfig object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from PayzenConfig object
        }
...
//
PayzenConfigTableMap::buildTableMap();
